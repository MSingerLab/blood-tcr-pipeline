...
#data <- Read10X(data.dir = "/Users/jacobluber/dfci_share/jacob/data/K468/K468_LN_5primeGEX_analysis/filtered_feature_bc_matrix")
#load an normalize data
data <- Read10X(data.dir = tenxdir)
data2 <- CreateSeuratObject(data, min.cells = 3,min.features=200, project = "test")
...
new.idents <- tcr$Clone
names(new.idents)=tcr$Barcode
working_so <- AddMetaData(working_so,new.idents,col.name="Clone")
